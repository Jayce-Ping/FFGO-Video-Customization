...
from videox_fun.data.dataset_transition import (ImageVideoDataset,
                                                ImageVideoSampler,
                                                get_random_mask)
from videox_fun.models import (AutoencoderKLWan, AutoencoderKLWan3_8, WanT5EncoderModel,
                              Wan2_2Transformer3DModel)
from videox_fun.pipeline import Wan2_2Pipeline, Wan2_2I2VPipeline, Wan2_2TI2VPipeline
from videox_fun.utils.discrete_sampler import DiscreteSampling
from videox_fun.utils.lora_utils_select import create_network, merge_lora, unmerge_lora
from videox_fun.utils.utils import get_image_to_video_latent, save_videos_grid



LORA_MODULES = ["k", "v", "q", "o", "ffn.0", "ffn.2"]
...
def log_validation(vae, text_encoder, tokenizer, transformer3d, network, config, args, accelerator, weight_dtype, global_step, h, w):
    try:
            ...
            with torch.no_grad():
                if args.train_mode != "normal":
                    with torch.autocast("cuda", dtype=weight_dtype):
                        ...
                        input_video, input_video_mask, _ = get_image_to_video_latent(args.validation_images[i] , None, video_length=video_length, sample_size=[h, w])
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=[512],
        help="Sample size of the video. Can be single int (square) or two ints (height width).",
    )
    parser.add_argument(
        "--image_sample_size",
        type=int,
    ...
    parser.add_argument(
        "--height", type=int, default=None, help="The height for input images, all images will be resized to this height"
    )
    parser.add_argument(
        "--width", type=int, default=None, help="The width for input images, all images will be resized to this width"
    )
    ...
    # Convert video_sample_size to tuple (height, width)
    if isinstance(args.video_sample_size, list):
        if len(args.video_sample_size) == 1:
            args.video_sample_size = (args.video_sample_size[0], args.video_sample_size[0])
        else:
            args.video_sample_size = tuple(args.video_sample_size[:2])
    elif isinstance(args.video_sample_size, int):
        args.video_sample_size = (args.video_sample_size, args.video_sample_size)

    if isinstance(args.image_sample_size, list):
        if len(args.image_sample_size) == 1:
            args.image_sample_size = (args.image_sample_size[0], args.image_sample_size[0])
        else:
            args.image_sample_size = tuple(args.image_sample_size[:2])
    elif isinstance(args.image_sample_size, int):
        args.image_sample_size = (args.image_sample_size, args.image_sample_size)
    return args
...
def main():
    ...
    print(f"lora_modules is {lora_modules}")
    ...
    with open(os.path.join(args.output_dir, "network_structure.txt"), "w") as f:
        f.write(str(network))

    # save transformer3d structure
    with open(os.path.join(args.output_dir, "transformer3d_structure.txt"), "w") as f:
        f.write(str(transformer3d))
    ...
    # save trainable parameters in a .txt file to check they are correct
    ...
    # video_sample_size is now a tuple (height, width)
    video_sample_size_min = min(args.video_sample_size)
    image_sample_size_min = min(args.image_sample_size)
    
    if args.fix_sample_size is not None and args.enable_bucket:
        args.video_sample_size = (
            max(max(args.fix_sample_size), args.video_sample_size[0]),
            max(max(args.fix_sample_size), args.video_sample_size[1])
        )
        video_sample_size_min = min(args.video_sample_size)
    ...
    if args.enable_bucket:
        aspect_ratio_sample_size = {key : [x / 512 * video_sample_size_min for x in ASPECT_RATIO_512[key]] for key in ASPECT_RATIO_512.keys()}
        ...
        def get_length_to_frame_num(token_length):
            if image_sample_size_min > video_sample_size_min:
                sample_sizes = list(range(video_sample_size_min, image_sample_size_min + 1, 128))
                if sample_sizes[-1] != image_sample_size_min:
                    sample_sizes.append(image_sample_size_min)
            else:
                sample_sizes = [image_sample_size_min]
        ...
        def collate_fn(examples):
            # 如果 video_sample_size 是非正方形，token_sample_size 也应该分开
            target_token_length = args.video_sample_n_frames * (args.video_sample_size[0] // spatial_compression_ratio) * (args.video_sample_size[1] // spatial_compression_ratio)
            ...
            if data_type == 'image':
                image_sample_size_min = min(args.image_sample_size)
                random_downsample_ratio = 1 if not args.random_hw_adapt else get_random_downsample_ratio(args.image_sample_size, image_ratio=[args.image_sample_size / video_sample_size_min], rng=rng)

                aspect_ratio_sample_size = {key : [x / 512 * image_sample_size_min / random_downsample_ratio for x in ASPECT_RATIO_512[key]] for key in ASPECT_RATIO_512.keys()}
                ...
                if args.random_hw_adapt:
                    if args.training_with_video_token_length:
                        ...
                        random_downsample_ratio = video_sample_size_min / local_video_sample_size
                    else:
                        random_downsample_ratio = get_random_downsample_ratio(
                                video_sample_size_min, rng=rng)
                        batch_video_length = args.video_sample_n_frames + sample_n_frames_bucket_interval
                else:
                ...
                aspect_ratio_sample_size = {key : [x / 512 * video_sample_size_min / random_downsample_ratio for x in ASPECT_RATIO_512[key]] for key in ASPECT_RATIO_512.keys()}
                aspect_ratio_random_crop_sample_size = {key : [x / 512 * video_sample_size_min / random_downsample_ratio for x in ASPECT_RATIO_RANDOM_CROP_512[key]] for key in ASPECT_RATIO_RANDOM_CROP_512.keys()}
            ...
            for example in examples:
                if args.fix_sample_size is not None:
                    ...
                    # print('pixel_values shape before resize:', pixel_values.shape) # T, C, H, W
                    # '''
                    # pixel_values shape before resize: torch.Size([81, 3, 640, 360])
                    # '''
                    # exit(0)
                    ...
                    # print(f"closest_size: {closest_size}, closest_ratio: {closest_ratio}, h: {h}, w: {w}")
                    # print("resize_size:", resize_size)
                    # '''
                    # closest_size: [320, 192], closest_ratio: 1.75, h: 640, w: 360
                    # resize_size: (341, 192)    
                    # '''
                    # exit(0)
    ...
    if accelerator.is_main_process:
        ...
        tracker_config.pop('image_sample_size')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            if epoch == first_epoch and step == 0:
                ...
                # print("pixel_values min max", torch.min(pixel_values), torch.max(pixel_values))
                # print("texts", texts)
                # '''
                # pixel_values torch.Size([1, 81, 3, 320, 192])
                # pixel_values min max tensor(-1.) tensor(0.9990)
                # texts ['ad23r2 the camera view suddenly changes to reveal, the women pick up the object'] 
                # '''
                # exit(0)
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.auto_tile_batch_size and args.training_with_video_token_length and zero_stage != 3:
                    token_h = args.video_sample_size[0] // spatial_compression_ratio  
                    token_w = args.video_sample_size[1] // spatial_compression_ratio
                    target_tokens = args.video_sample_n_frames * token_h * token_w

                    if target_tokens // 16 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                    ...
                    elif target_tokens // 4 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                ...
                if args.keep_all_node_same_token_length:
                    token_h = args.video_sample_size[0] // spatial_compression_ratio
                    token_w = args.video_sample_size[1] // spatial_compression_ratio
                    min_token_size = min(token_h, token_w)
                    if min_token_size > 256:
                        numbers_list = list(range(256, min_token_size + 1, 128))
                        if numbers_list[-1] != min_token_size:
                            numbers_list.append(min_token_size)
                    else:
                        numbers_list = [256]
                    aspect_ratio = token_h / token_w

                    numbers_list = [int(_number * _number * aspect_ratio * args.video_sample_n_frames) for _number in numbers_list]
                ...
                with torch.no_grad():
                    ...
                    if vae_stream_1 is not None:
                        ...
                        latents = _batch_encode_vae(pixel_values)
                        # print("pixel_values", pixel_values.shape, "-> latents", latents.shape)
                        # '''
                        # pixel_values torch.Size([1, 3, 81, 320, 192]) -> latents torch.Size([1, 48, 21, 20, 12])
                        # '''
                        # exit(0)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if args.validation_prompts is not None and global_step % args.validation_steps == 0 or global_step == 0:
                        log_validation(
                            vae,
                            text_encoder,
                            tokenizer,
